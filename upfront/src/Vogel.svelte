<script>
    ...
    async function callVoguel(fileStr) {
        try {
            const response = await fetch('http://localhost:8000/voguel?file_str=' + encodeURIComponent(fileStr));
            if (!response.ok) {
...
// 7,10,20,7, 18`
        ...
        const apiUrl = 'http://localhost:8000/voguel';
        callVoguel(file)
        .then(result => {
            logs = result.result;
            ...
            console.log('Voguel API response:', result);
        })
        .catch(error => {
...
<main>
    ...
    <div class="action-buttons">
        <button class="solve" on:click={() => solve()}>VOGEL</button>
    </div>
